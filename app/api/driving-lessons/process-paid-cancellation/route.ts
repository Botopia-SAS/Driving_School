 ...
 * Este endpoint ya no usa Stripe para el pago de cancelación.
 *
 * This endpoint:
 ...
 * @param {string} orderId - ID de orden para seguimiento de pago
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { studentId, instructorId, date, start, end, slotId, orderId } =
      await request.json();

    console.log(
      "💰 [DRIVING LESSON PAID CANCEL] Processing paid cancellation:",
      {
        studentId,
        instructorId,
        date,
    ...
    console.log(
      "🔍 [DRIVING LESSON PAID CANCEL] Found instructor:",
      instructor.name
    );
    ...
    if (instructor.schedule_driving_lesson) {
      if (slotId) {
        ...
        foundSlot = instructor.schedule_driving_lesson.find(
          (slot: ScheduleSlot) => slot._id === slotId
        );
        console.log(
          "🔍 [DRIVING LESSON PAID CANCEL] Searching by slotId:",
          slotId,
          "Found:",
          !!foundSlot
        );
        ...
        console.log(
          "🔍 [DRIVING LESSON PAID CANCEL] Searching by date/time/studentId. Found:",
          !!foundSlot
        );
      }
    ...
    if (!foundSlot) {
      console.log(
        "❌ [DRIVING LESSON PAID CANCEL] Slot not found in instructor schedules"
      );
      return NextResponse.json({ error: "Slot not found" }, { status: 404 });
    }

    console.log(
      "✅ [DRIVING LESSON PAID CANCEL] Found slot:",
      foundSlot,
      "in",
      scheduleType
    );
    ...
    if (foundSlot.studentId !== studentId) {
      console.log(
        "❌ [DRIVING LESSON PAID CANCEL] Slot does not belong to student"
      );
      return NextResponse.json(
    ...
    console.log(
      "📋 [DRIVING LESSON PAID CANCEL] Original slot before cancellation:",
    ...
    console.log(
      "🆕 [DRIVING LESSON PAID CANCEL] Creating new slot with ID:",
      slotIdString
    );
    ...
    if (
      scheduleType === "schedule_driving_lesson" &&
      instructor.schedule_driving_lesson
    ) {
      instructor.schedule_driving_lesson.push(newAvailableSlot);
      console.log(
        "📊 [DRIVING LESSON PAID CANCEL] Added to schedule_driving_lesson. Total slots before save:",
    ...
    console.log(
      "✅ [DRIVING LESSON PAID CANCEL] Slot marked as cancelled and new available slot created"
    );

    // 3. Find the user and move booking from booked to cancelled
    const user = await User.findById(studentId);
    console.log(
      "🔍 [DRIVING LESSON PAID CANCEL] User found for moving booking:",
      {
        userId: user?._id,
        drivingLessonBookingsCount: user?.driving_lesson_bookings?.length || 0,
        drivingLessonCancelledCount:
          user?.driving_lesson_cancelled?.length || 0,
      }
    );
    ...
    if (user && user.driving_lesson_bookings) {
      ...
      if (slotId) {
        bookingIndex = user.driving_lesson_bookings.findIndex(
      ...
      if (bookingIndex === -1) {
        ...
        console.log(
          "🔍 [DRIVING LESSON PAID CANCEL] Searching by date/time/instructor. Found index:",
          bookingIndex
        );
      }

      console.log(
        "🔍 [DRIVING LESSON PAID CANCEL] Looking for booking to move:",
        {
          slotId,
          instructorId,
          date,
      ...
      if (bookingIndex !== -1) {
        ...
        console.log(
          "✅ [DRIVING LESSON PAID CANCEL] Found booking to move:",
          bookingToMove
        );
        ...
        console.log(
          "🔄 [DRIVING LESSON PAID CANCEL] Removed booking from user:",
          updateResult
        );
        ...
        console.log(
          "✅ [DRIVING LESSON PAID CANCEL] Added to cancelled bookings:",
          cancelledUpdateResult
        );
        console.log(
          "💰 [DRIVING LESSON PAID CANCEL] Paid cancellation processed - no credit granted"
        );
      } else {
        console.log(
          "❌ [DRIVING LESSON PAID CANCEL] Booking not found in user bookings"
        );
      }
    } else {
      console.log(
        "❌ [DRIVING LESSON PAID CANCEL] User not found or no bookings"
      );
    }
    ...
    try {
      ...
      console.log(
        "📡 [DRIVING LESSON PAID CANCEL] SSE broadcast sent for instructor:",
        instructorId
      );
    } catch (sseError) {
      console.warn(
        "⚠️ [DRIVING LESSON PAID CANCEL] SSE broadcast failed:",
        sseError
      );
    }
