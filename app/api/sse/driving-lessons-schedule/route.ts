...
import {
  ...
  sendScheduleUpdate as sendUpdate
} from "@/lib/sse-driving-lessons-broadcast";
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      console.log(`🔌 Starting SSE connection ${connectionId} for driving lessons`);
      ...
      const interval = setInterval(async () => {
        ...
        try {
          await sendUpdate(controller, encoder, instructorId, connectionId);
        } catch (error) {
...
async function sendInitialData(controller: ReadableStreamDefaultController, encoder: TextEncoder, instructorId: string, connectionId: string) {
  try {
    ...
    console.log(`📡 Sending initial driving lessons schedule for instructor ${instructorId} (${connectionId}):`, schedule.length, 'slots');
